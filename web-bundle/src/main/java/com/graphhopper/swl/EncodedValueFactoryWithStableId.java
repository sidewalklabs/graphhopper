...
public class EncodedValueFactoryWithStableId extends DefaultEncodedValueFactory {
    @Override
    public EncodedValue create(String name) {
        if (name.startsWith("stable-id-byte-")) {
            return new UnsignedIntEncodedValue(name, 8, false);
        } else if (name.startsWith("reverse-stable-id-byte-")) {
            return new UnsignedIntEncodedValue(name, 8, false);
        } else {
